#' Initialize and setup the Seurat2 object
#'
#' Initializes the Seurat2 object and some optional filtering
...
#' cell's column name
#' @param meta.data Additional metadata to add to the Seurat2 object. Should be a data frame where
...
#' @param ... Ignored
#'
#' @return Returns a Seurat2 object with the raw data stored in object@@raw.data.
...
#' @examples
#' pbmc_raw <- read.table(
#'   file = system.file('extdata', 'pbmc_raw.txt', package = 'Seurat2'),
#'   as.is = TRUE
#' )
#' pbmc_small <- CreateSeurat2Object(raw.data = pbmc_raw)
#' pbmc_small
#'
CreateSeurat2Object <- function(
  raw.data,
  project = "Seurat2Project",
  min.cells = 0,
  min.genes = 0,
  is.expr = 0,
  ...
  seurat2.version <- packageVersion("Seurat2")
  object <- new(
    Class = "seurat2",
    raw.data = raw.data,
    is.expr = is.expr,
    project.name = project,
    version = seurat2.version
  )
  ...
  parameters.to.store <- as.list(x = environment(), all = TRUE)[names(formals("CreateSeurat2Object"))]
  parameters.to.store$raw.data <- NULL
  parameters.to.store$meta.data <- NULL
  object <- SetCalcParams(
    object = object,
    calculation = "CreateSeurat2Object",
    ... = parameters.to.store
  )
...
#' seurat2_object = CreateSeurat2Object(raw.data = expression_matrix)
#' }
#'
...
#'
#' @return Returns a seurat2 object with object@@scale.data updated with scaled and/or centered data.
#'
...
#'
#' @return Returns a seurat2 object with object@@scale.data updated with scaled
...
#' @importFrom utils txtProgressBar setTxtProgressBar
#'
#' @return Returns a Seurat2 object, placing variable genes in object@@var.genes.
