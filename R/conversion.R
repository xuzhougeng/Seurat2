...
#' Convert Seurat2 objects to other classes and vice versa
#'
...
#'
#' @describeIn Convert Convert a Seurat2 object
#'
...
#'
#' @export
#' @method Convert seurat2
#'
Convert.seurat2 <- function(
  from,
  to,
  filename,
  chunk.dims = 'auto',
  ...
  object.to <- switch(
    ...
    'sce' = {
      ...
      sce <- if (class(from@raw.data) %in% c("matrix", "dgTMatrix")) {
        ...
        stop("Invalid class stored in seurat2 object's raw.data slot")
      }
      if (class(from@data) %in% c("matrix", "dgTMatrix")) {
        ...
        stop("Invalid class stored in seurat2 object's data slot")
      }
    ...
    'anndata' = {
      ...
      raw <- switch(
        ...
        "data" = from@data,
        stop("Invalid Seurat2 data slot. Please choose one of: raw.data, data")
      )
      ...
      X <- switch(
        ...
        stop("Invalid Seurat2 data slot. Please choose one of: data, scale.data")
      )
    ...
    stop(paste0("Cannot convert Seurat2 objects to class '", to, "'"))
  )
  return(object.to)
}
...
#'
#' @describeIn Convert Convert from SingleCellExperiment to a Seurat2 object
#' @export
...
Convert.SingleCellExperiment <- function(
  ...
  object.to <- switch(
    ...
    'seurat2' = {
      ...
      seurat2.object <- CreateSeurat2Object(raw.data = raw.data, meta.data = meta.data)
      seurat2.object@data <- data
      if (length(x = SingleCellExperiment::reducedDimNames(from)) > 0) {
        for (dr in SingleCellExperiment::reducedDimNames(from)) {
          seurat2.object <- SetDimReduction(
            object = seurat2.object,
            reduction.type = dr,
...
#' @param X.slot Seurat2 slot to transfer anndata X into. Default is scale.data
#' @param raw.slot Seurat2 slot to transfer anndata raw into. Default is data
#' @describeIn Convert from Anndata file to a Seurat2 object
...
Convert.anndata.base.AnnData <- function(
  ...
  object.to <- switch(
    ...
    'seurat2' = {
      ...
      seurat2.object <- CreateSeurat2Object(
        raw.data = raw.data.matrix,
        meta.data = meta.data
      )
      seurat2.object <- SetAssayData(
        object = seurat2.object,
        assay.type = "RNA",
        slot = X.slot,
      ...
      for (dr in drs) {
        ...
        rownames(x = dr.embed) <- seurat2.object@cell.names
        seurat2.object <- SetDimReduction(
          object = seurat2.object,
