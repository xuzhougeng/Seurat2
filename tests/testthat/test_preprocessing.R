# Tests for functions dependent on a seurat2 object
set.seed(42)

pbmc.file <- system.file('extdata', 'pbmc_raw.txt', package = 'Seurat2')
pbmc.test <- as(as.matrix(read.table(pbmc.file, sep = "\t", row.names = 1)), "dgCMatrix")

# Tests for object creation (via CreateSeurat2Object)
# --------------------------------------------------------------------------------
...
object <- CreateSeurat2Object(raw.data = pbmc.test,
                             normalization.method = "LogNormalize",
                             do.scale = T,
                             meta.data = fake.meta.data,
                             # save.raw = F,
                             display.progress = F)
test_that("object initialization actually creates seurat2 object", {
  expect_is(object, "seurat2")
})
